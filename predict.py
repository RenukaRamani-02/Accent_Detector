...
def predict_accent(audio_file):
    """Predict accent label and confidence from audio file."""
    ...
    if hasattr(accent_model, "predict_proba"):
        ...
        confidence = float(np.max(proba[0])) * 100  # percentage
    else:
        confidence = 100.0
...
def predict_age(audio_file):
    """Predict age group label and confidence from audio file."""
